...
#include "dashboardwindow.h" // Include the header for the DashboardWindow
...
SignInWindow::SignInWindow(QWidget *parent) : QWidget(parent) {
    ...
    // Style input fields
    usernameEdit->setFixedSize(300, 40);  // Ensure a fixed width
    ...
    QLabel *registerLabel = new QLabel("<a href='#'>Register Now</a>", this);
    ...
    bottomLinksLayout->addWidget(registerLabel); // Register now side by side with forgot password

    // Centered Form Layout
    QVBoxLayout *formLayout = new QVBoxLayout();
    formLayout->setAlignment(Qt::AlignCenter);
    formLayout->addWidget(logoContainer, 0, Qt::AlignCenter);  // Keep the logo centered
    formLayout->addSpacing(10);
    formLayout->addWidget(usernameEdit);
    formLayout->addWidget(passwordEdit);
    formLayout->addWidget(loginButton);
    formLayout->addLayout(bottomLinksLayout);  // Add register & forgot password links
    ...
    // Exit Button (Small Red Rectangle with "X")
...
void SignInWindow::onLoginClicked() {
    // After successful login, open the Dashboard window
