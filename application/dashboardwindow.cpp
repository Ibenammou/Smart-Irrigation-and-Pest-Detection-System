...
void DashboardWindow::setupLeftSidebar() {
    ...
    // Add background image to sidebar with transparency
    leftSidebar->setStyleSheet("QWidget { background-color: #57C78F; background-image: url(:/images/ack.jpeg); background-repeat: no-repeat; background-position: center; background-size: 80px 80px; }");
    ...
    pestDisplayButton->setStyleSheet("background-color: #57C78F; color: #FFFFFF; border-radius: 10px; padding: 10px;");
...
void DashboardWindow::setupMainSection() {
    ...
    // Make columns fit without horizontal scroll
    parcelDetailsTable->verticalHeader()->setVisible(false);
    parcelDetailsTable->setWordWrap(false);

    // Set specific resize modes for each column
    parcelDetailsTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::ResizeToContents); // Parcelle
    parcelDetailsTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch); // Dernière Irrigation (needs more space)
    parcelDetailsTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents); // Consommation
    parcelDetailsTable->horizontalHeader()->setSectionResizeMode(3, QHeaderView::ResizeToContents); // État
    parcelDetailsTable->horizontalHeader()->setSectionResizeMode(4, QHeaderView::ResizeToContents); // Humidité
    parcelDetailsTable->horizontalHeader()->setSectionResizeMode(5, QHeaderView::ResizeToContents); // Nutriments
    parcelDetailsTable->horizontalHeader()->setSectionResizeMode(6, QHeaderView::Stretch); // Détails d'alerte (needs more space)

    parcelDetailsTable->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    ...
    basilicSeries->append(1, 40);
    basilicSeries->append(2, 35);
    basilicSeries->append(3, 50);
    basilicSeries->append(4, 45);
    basilicSeries->append(5, 30);
    basilicSeries->append(6, 55);
    basilicSeries->append(7, 40);

    QLineSeries *pommierSeries = new QLineSeries();
    pommierSeries->setName("🍏 Pommier");
    pommierSeries->append(1, 50);
    pommierSeries->append(2, 45);
    pommierSeries->append(3, 60);
    pommierSeries->append(4, 35);
    pommierSeries->append(5, 70);
    pommierSeries->append(6, 40);
    pommierSeries->append(7, 55);

    QLineSeries *orangerieSeries = new QLineSeries();
    orangerieSeries->setName("🍊 Orangerie");
    orangerieSeries->append(1, 30);
    orangerieSeries->append(2, 25);
    orangerieSeries->append(3, 45);
    orangerieSeries->append(4, 20);
    orangerieSeries->append(5, 35);
    orangerieSeries->append(6, 50);
    orangerieSeries->append(7, 30);
